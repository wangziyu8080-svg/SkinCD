...
def _patch_qwen25_prepare_inputs_for_generation_cd():
    ...
    def prepare_inputs_for_generation_cd(self, input_ids, **kwargs):
        ...
        # Strip VCD meta kwargs so they never leak into model forward inputs.
        ...
        # Only replace image features in prefill. During decode steps with cache,
        # forcing pixel_values each token drifts CD away from clean even for same image.
        # Optional experiment: keep images_cd for the first decode step after prefill.
...
def build_default_dola_layers(model_config) -> List[int]:
    num_layers = getattr(getattr(model_config, "text_config", None), "num_hidden_layers", None)
...
def eval_model(args):
    ...
    label_token_sequences = build_label_token_sequences(processor.tokenizer, labels) if args.constrain_labels else []
    ...
    # Keep decoding mode aligned with base by default.
    # Forcing do_sample=True under use_cd introduces extra sampling warpers
    # (temperature/top-p/top-k), which can dominate classification behavior.
        ...
        for idx, row in enumerate(tqdm(rows), 1):
            ...
            with torch.inference_mode():
                ...
                if args.use_cd:
                    ...
                    if args.cd_debug_path:
                        generate_kwargs.update(
                            {
                                "cd_debug_path": os.path.expanduser(args.cd_debug_path),
                                "cd_debug_sample_id": image_rel,
                                "cd_debug_topk": args.cd_debug_topk,
                            }
                        )
